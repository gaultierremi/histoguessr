"use client";
import TimelineYearPicker from "@/components/TimelineYearPicker";
        ...
        <div className="rounded-3xl border border-gray-800 bg-gray-900 p-6">
          ...
          {room.status === "playing" ? (
            room.game_mode === "timeline" ? (
              ...
              <GameView room={room} participants={participants} />
            )
...
function LobbyView({
  ...
  return (
    <div className="mt-8 grid gap-6 md:grid-cols-[1fr_260px]">
      ...
      <aside className="rounded-3xl border border-gray-800 bg-gray-950 p-5">
...
function TimelineMultiplayerView({
  room,
  participants,
}: {
  room: Room;
  ...
  useEffect(() => {
    async function loadEvent() {
      ...
      setAnswersCount(0);
      setBonusPop(null);
      setActiveEventIndex(room.current_question_index);
  ...
  useEffect(() => {
    ...
    if (activeEventIndex !== room.current_question_index) return;
    if (!event) return;
  ...
  useEffect(() => {
    ...
    setShowReveal(true);
  }, [timeLeft, activeEventIndex, room.current_question_index, event]);
  ...
  }, [showReveal, activeEventIndex, room, supabase]);
  ...
  return (
    <div className="mt-8 grid gap-6 md:grid-cols-[1fr_260px]">
      <section className="relative rounded-3xl border border-gray-800 bg-gray-950 p-6">
        {bonusPop && (
        ...
        <TimelineYearPicker
          years={[event.year - 300, event.year, event.year + 300]}
          ...
          return (
            ...
            >
              ...
              <div className="relative">
                <div
                  className={index === 0 ? "animate-bounce text-6xl" : "text-5xl"}
                >
                  {medal}
                </div>
